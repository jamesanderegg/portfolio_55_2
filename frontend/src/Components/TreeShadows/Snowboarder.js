...
const boardMaterial = new MeshStandardMaterial({ color: "#8B4513" });
const beamOuterMaterial = new ShaderMaterial({
  ...
  side: DoubleSide,
  uniforms: {
    beamColor: { value: new Vector3(0.62, 0.86, 1.0) },
    maxAlpha: { value: 0.5 },
  },
  vertexShader: `
    varying vec2 vUv;
...
const beamCoreMaterial = new ShaderMaterial({
  ...
  side: DoubleSide,
  uniforms: {
    beamColor: { value: new Vector3(0.9, 0.97, 1.0) },
    maxAlpha: { value: 0.36 },
  },
...
function Snowboarder({
  ...
  useFrame(({ clock }) => {
    if (curveRef.current && !hasExited.current) {
      ...
      snowboarderRef.current.position.copy(riderPosition);
      snowboarderRef.current.lookAt(riderLookTarget);
  ...
  return (
    <>
      <mesh ref={snowboarderRef} castShadow>
        <sphereGeometry args={[0.8, 15, 12]} rotation={[0, 0, Math.PI / 2]} />
        <meshStandardMaterial color={color} />
        <mesh position={[0, -0.85, 0]} material={boardMaterial}>
          <boxGeometry args={[0.825, 2.2, 4.4]} />
        </mesh>
        <mesh
          position={[0, 0, 8]}
